...
class Population:
    ...
    def prepPopulation(self):
        ...
        self.currentGeneration = []
        # Startchromo zur Startpopulation einmal hinzufügen:
        if self.preprocessedInit == True:
            c = Chromosome()
            c.geneMinValues = self.chromoMinValues
            c.geneMaxValues = self.chromoMaxValues
            for i in range(len(self.startchromo)):
                c.genes.append(self.startchromo[i])
        ...
        for i in range(self.numChromosomes):
            ...
            # Using the preprocessed parameters?
            if self.preprocessedInit == True:
    ...
    def select_EliteRanked(self):
        ...
        """
        self.currentGeneration = sorted(self.currentGeneration)
        elites = self.currentGeneration[-self.selectionSize:]
        genSize = len(elites)
        # formula for sum of natural numbers
        sumRanks = genSize * (genSize + 1) / 2
        wheelPosition = self.generator.uniform(0, sumRanks)

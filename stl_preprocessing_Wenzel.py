...
def startparam(input_file,poly_order,savgol_window_quotient,max_distance):
    ...
    # Comment_DKu_Wenzel: todo: clean up. What is still needed? Large list should be deleteted → Maybe bigger stl-files possible
    ...
    #Interpolation
    grid_lin_interpolation, x_list_interpol_trendline, y_list_interpol_trendline = interpolate_start_geometrie(
        max_distance)

    draw_start_point = True
    if draw_start_point:
        startpoint_project_to_trendline_plan = bend_pts_xyz_global_3D[0][:]
        endpoint_project_to_trendline_plan = bend_pts_xyz_global_3D[-1][:]
...
def interpolate_start_geometrie(max_distance, grid_resolution=3000j):
    ...
    # Tilted line, line given by two points xdata and ydata. Startpoint and one additional Point for the direction (Endpoint). First values are drawn into the colormap.
    ...
    calculate_iteratively_the_tilted_bendingpoints(alpha_angle_list, grid_ressolution_int, grid_x, max_distance, max_x,
    ...
    return z_grid_values_linear, new_bending_direction_points_tilted_KOS_stacked[0][:, 0], new_bending_direction_points_tilted_KOS_stacked[0][:, 2]
...
def show_results_2D_Plots_and_Colormap(max_x, max_y, min_x, min_y, new_bending_direction_points_tilted_KOS_left_stacked,
    ...
    # In the different tilted KOS the x-Values don´t allign. Correcting it here for nicer plot
...
def calculate_iteratively_the_tilted_bendingpoints(alpha_angle_list, grid_ressolution_int, grid_x, max_distance, max_x,
                                                   max_y, min_x, min_y, x_0_grid_point_index, x_values_trim_stacked,
                                                   xdata_list, y_0_grid_point_index, y_values_trim_stacked, ydata_list,
                                                   z_grid_values_linear):
...
def calc_tilted_bending_points(grid_ressolution_int, grid_x, max_y, min_y, max_x, min_x, y_0_grid_point_index,
    ...
    x_start_index_left = calc_start_end_point_side_in_trendline_KOS(True, delta_length_end_bend, delta_length_start_bend,
                                                                    dx, dy, end_point_xyz_trendline_data, grid_ressolution_int,
                                                                     start_point_xyz_trendline_data, width, x_0_grid_point_index,
    ...
    x_end_index_right, \
    x_start_index_right = calc_start_end_point_side_in_trendline_KOS(False, delta_length_end_bend, delta_length_start_bend, dx, dy, end_point_xyz_trendline_data, grid_ressolution_int,
                                                                     start_point_xyz_trendline_data, width, x_0_grid_point_index, trendline_new_direction_current_KOS[0], y_0_grid_point_index,
                                                                      z_grid_values_linear, trendline_new_direction_current_KOS[1])

    global bend_pts_xz_local_left
    ...
    new_bending_direction_points_left_current_KOS, y_intercept_left, x_values_indizes_trim_left, x_values_trim_left, y_values_indizes_trim_left, y_values_trim_left = calc_new_direction_points_on_surface(
        start_point_xyz_trendline_data_left, dy, grid_ressolution_int, x_slope, x_values, y_0_grid_point_index, z_grid_values_linear,
    ...
    new_bending_direction_points_tilted_KOS_left, new_bending_direction_points_on_surface_global_KOS_left, start_end_point_xy_data_global_KOS_left, trendline_new_direction_global_KOS_left = new_direction_points_in_global_and_tilted_KOS(
        start_point_xyz_trendline_data_left, end_point_xyz_trendline_data_left, y_intercept_left, new_bending_direction_points_left_current_KOS,
    ...
    global bend_pts_xz_local_right
    new_bending_direction_points_right_current_KOS, y_intercept_right, x_values_indizes_trim_right, x_values_trim_right, y_values_indizes_trim_right, y_values_trim_right = calc_new_direction_points_on_surface(
        end_point_xyz_trendline_data_right, dy, grid_ressolution_int, x_slope, x_values, y_0_grid_point_index, z_grid_values_linear,
    ...
    new_bending_direction_points_tilted_KOS_right, new_bending_direction_points_on_surface_global_KOS_right, start_end_point_xy_data_global_KOS_right, trendline_new_direction_global_KOS_right = new_direction_points_in_global_and_tilted_KOS(
        start_point_xyz_trendline_data_right, end_point_xyz_trendline_data_right, y_intercept_right, new_bending_direction_points_right_current_KOS,
        trendline_new_direction_current_KOS)
    ...
    global trendline_new_direction_global_KOS,bend_pts_xz_local
    new_bending_direction_points_on_surface_global_KOS, y_intercept, x_values_indizes_trim, x_values_trim, y_values_indizes_trim, y_values_trim = calc_new_direction_points_on_surface(
        start_point_xyz_trendline_data, dy, grid_ressolution_int, x_slope, x_values, y_0_grid_point_index, z_grid_values_linear,
        x_start_index, x_end_index)

    new_bending_direction_points_tilted_KOS, new_bending_direction_points_on_surface_global_KOS, start_end_point_xy_data_global_KOS, trendline_new_direction_global_KOS = new_direction_points_in_global_and_tilted_KOS(
        start_point_xyz_trendline_data, end_point_xyz_trendline_data, y_intercept, new_bending_direction_points_on_surface_global_KOS,
        trendline_new_direction_current_KOS)
    ...
    edge_line_global_current_direction = [
        (bend_pts_xyz_global[:][:2]+np.multiply(edge_directions[:][:2], 50)),(bend_pts_xyz_global[:][:2]-np.multiply(edge_directions[:][:2], 50))]
    #edge_line_global.append([edge_line_global_current_direction[0][0],edge_line_global_current_direction[1][0]])
    ...
    if len(edge_directions) > 2:
        rotated_x_direction_point_in_trendline_KOS = np.asarray([bend_pts_xyz_global[1] + np.multiply(rotated_x_direction_around_edge_current_direction[0],5),bend_pts_xyz_global[1]])
        rotated_x_direction_around_edge_trendline_KOS_current_direction = translation_Points_from_OLD_to_trendline_KOS(rotated_x_direction_point_in_trendline_KOS,
                                                                                               trendline_global_KOS,
                                                                                               center_point_of_cloud_weighted)
        # xdata,ydata gibt die Richtung der nächsten Biegerichtung an
  ...
  # Cut bendingpoint and xy-line in trendlineplot to bendpoint0-bendpoint1


    end_index_pts_global = [i for i, x in enumerate(new_bending_direction_points_tilted_KOS[:,0]) if x >= bend_pts_xz_local[1][0]][0]
    end_index_pts_global_left = [i for i, x in enumerate(new_bending_direction_points_tilted_KOS_left[:,0]) if x >= bend_pts_xz_local_left[1][0]][0]
    end_index_pts_global_right = [i for i, x in enumerate(new_bending_direction_points_tilted_KOS_right[:,0]) if x >= bend_pts_xz_local_right[1][0]][0]

    # Trim points on surface

    # Global 3D-Plot
    global new_bending_direction_points_on_surface_global_KOS_stacked,new_bending_direction_points_on_surface_global_KOS_left_stacked,new_bending_direction_points_on_surface_global_KOS_right_stacked
    new_bending_direction_points_on_surface_global_KOS_stacked.append(new_bending_direction_points_on_surface_global_KOS[:][:end_index_pts_global])
    new_bending_direction_points_on_surface_global_KOS_right_stacked.append(new_bending_direction_points_on_surface_global_KOS_right[:][:end_index_pts_global_right])
    new_bending_direction_points_on_surface_global_KOS_left_stacked.append(new_bending_direction_points_on_surface_global_KOS_left[:][:end_index_pts_global_left])

    # local 2D-Plot
    global new_bending_direction_points_tilted_KOS_stacked, new_bending_direction_points_tilted_KOS_left_stacked, new_bending_direction_points_tilted_KOS_right_stacked, \
        bend_pts_xz_local_stacked, bend_pts_xz_local_right_stacked, bend_pts_xz_local_left_stacked
    new_bending_direction_points_tilted_KOS_stacked.append(new_bending_direction_points_tilted_KOS[:end_index_pts_global])
    new_bending_direction_points_tilted_KOS_left_stacked.append(new_bending_direction_points_tilted_KOS_left[:end_index_pts_global])
    new_bending_direction_points_tilted_KOS_right_stacked.append(new_bending_direction_points_tilted_KOS_right[:end_index_pts_global])
...
def calc_bending_parameters_with_bendpoints(bend_pts_xyz_global, bend_pts_xyz_global_left, bend_pts_xyz_global_right,bend_pts_xyz_trendline,
    ...
    for i in range(1, (len(edge_directions) - 1)):
        ...
        edge_directions_trendlineKOS = translation_Points_from_OLD_to_trendline_KOS(np.asarray(edge_directions),trendline_patch,
                                                                                    np.asarray([0, 0, 0]))
    ...
    if len(edge_directions)>2:
        ...
        global x_y_direction_tape_in_trendline_KOS_stacked
        x_y_direction_tape_in_trendline_KOS_stacked.append(translation_Points_from_OLD_to_trendline_KOS(x_y_stacked,
                                                                                                        trendline_global_KOS,
                                                                                                        np.asarray([0,0,0])))
...
def calc_edge_directions(bend_pts_xyz_global_left, bend_pts_xyz_global_right):
    ...
    edge_directions = []
    if len(bend_pts_xyz_global_left) > 3:
        edges = 3
    else:
        edges = len(bend_pts_xyz_global_left)
...
def show_startstrip(bestPatch_patternpoints,patch_start,patch_end):
    ...
    for k in range(len(x_direction_list_global_KOS)):
            axes.plot([bend_pts_xyz_global_3D[k+1][0], bend_pts_xyz_global_3D[k+1][0] + 50 * x_direction_list_global_KOS[k][0]],
                      ...
                      [bend_pts_xyz_global_3D[k+1][2], bend_pts_xyz_global_3D[k+1][2] + 50 * x_direction_list_global_KOS[k][2]], label='Edge', c='red')
    ...
    for i in range(len(edge_line_global)):
        axes.plot([edge_line_global[i][0][0], edge_line_global[i][1][0]],
                ...
                [edge_line_global[i][0][2], edge_line_global[i][1][2]], label='Edge',

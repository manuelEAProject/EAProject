...
####MAIN
def startparam(input_file, max_distance):
    print(
    ...
    del tri_areas, point_cloud_tri_centerpoints_weighted

    #Interpolation
    interpolate_start_geometrie(max_distance)
    ...
    for i in range(len(new_bending_direction_points_tilted_KOS_stacked)):
        for j in range(1,len(new_bending_direction_points_tilted_KOS_stacked[i])):
            ...
            distance = calc_distance_between_two_points(new_bending_direction_points_tilted_KOS_stacked[i][j - 1],new_bending_direction_points_tilted_KOS_stacked[i][j])
...
def calc_bend_pts(max_distance, x_z_surface_point, y_smooth):
    ...
    while insert_pts:
        ...
        bend_pts_xz = np.insert(bend_pts_xz, -1,
                            np.array([x_z_surface_point[max_divergence[1]][0],
...
def interpolate_start_geometrie(max_distance, grid_resolution=2000j):
...
def translation_Points_from_OLD_to_trendline_KOS(points_in_old_KOS, new_trendline_axis_in_old_KOS,
...
def calc_tilted_bending_points(grid_ressolution_int, grid_x, max_y, min_y, max_x, min_x, y_0_grid_point_index,
    ...
    if len(edge_directions) > 2:
        ...
        rotated_x_direction_around_edge_trendline_KOS_current_direction = translation_Points_from_OLD_to_trendline_KOS(
  ...
  # Cut bendingpoint and xy-line in trendlineplot to bendpoint0-bendpoint1
    ...
    try: end_index_pts_global = [i for i, x in enumerate(new_bending_direction_points_tilted_KOS[:,0]) if x >= bend_pts_xz_local[1][0]][0]
    except: end_index_pts_global = -1
    try: end_index_pts_global_left = [i for i, x in enumerate(new_bending_direction_points_tilted_KOS_left[:,0]) if x >= bend_pts_xz_local_left[1][0]][0]
    except: end_index_pts_global_left = -1
    try: end_index_pts_global_right = [i for i, x in enumerate(new_bending_direction_points_tilted_KOS_right[:,0]) if x >= bend_pts_xz_local_right[1][0]][0]
    except: end_index_pts_global_right = -1
...
def calc_bending_parameters_with_bendpoints(bend_pts_xyz_global, bend_pts_xyz_global_left, bend_pts_xyz_global_right,bend_pts_xyz_trendline,
    ...
    for i in range(1, (len(edge_directions) - 1)):
        ...
        edge_directions_trendlineKOS = translation_Points_from_OLD_to_trendline_KOS(np.asarray(edge_directions),
                                                                                    trendline_patch,
                                                                                    np.asarray([0, 0, 0]))
        ...
        side_directions_rotated = translation_Points_from_OLD_to_trendline_KOS(side_directions, trendline_patch,
                                                                               np.asarray([0, 0, 0]))
    ...
    if len(edge_directions)>2:
        ...
        x_y_direction_tape_in_trendline_KOS_stacked.append(
            translation_Points_from_OLD_to_trendline_KOS(x_y_stacked, trendline_global_KOS, np.asarray([0, 0, 0])))
...
def new_direction_points_in_global_and_tilted_KOS(start_point_drawn, end_point_drawn, y_intercept,
    ...
    start_end_point_drawn_global_KOS = translation_Points_from_OLD_to_trendline_KOS(start_end_point_current_KOS,
                                                                                    trendline_global_KOS,
                                                                                    center_point_of_cloud_weighted,
                                                                                    True)
